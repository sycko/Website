...
Emre (voice-over): There are different processes that turn data into sound. It could be done digitally using softwares and algorithms, or analogously using hardware equipment (capacitors, resistors, etc.)
 
If it were done analogously, the hardware would require a sensor to receive data. This data can be anything as long as there is a sensor to read it. In the electronics lab we got to build a simple light controlled oscillator that played higher frequencies in a brighter environment. In this case the sensor was a photoresistor.
 
Data sonification using digital methods is much more easy and can be used to represent many more types of data including text and images. Instead of using hardware, this method relies on computer programs to read, analyze, and transform data into sound.
